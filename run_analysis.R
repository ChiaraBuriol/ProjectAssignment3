...
# 5.1 Make second tidy data set: UCI_HR_avg
UCI_HAR_avg <- UCI_HAR_Mean_Std %>%
             group_by(subject_id,activity_id, activity_type) %>%
             summarise_each(funs(if(is.numeric(.)) mean(., na.rm = TRUE) else first(.))) 

#5.2 Write UCI_HAR_avg in txt file

write.table(UCI_HAR_avg, "UCI_HAR_avg.txt", row.name=FALSE)
